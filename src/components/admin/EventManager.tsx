...
const resolveEventKeys = (adminProfile: any, isSuper: boolean): string[] => {
  if (isSuper) return []; // empty = "all" — handled separately in useEffect
  ...
  roles.forEach((role: string) => {
    ...
    COMPETITIONS_DATA.forEach(c => {
      ...
      // Guard: sfx.includes(x) is ONLY valid when x is non-empty — '' includes any string
      const matches =
        (slug   && slug.includes(sfx))   ||   // e.g. 'param-x-26' includes 'param-x'
        (slug   && sfx.includes(slug.replace('-26', ''))) || // reverse: 'param-x-26' slug stripped
        (id     && id.includes(sfx))     ||   // e.g. 'cmp-26-btg-bgmi-001' includes 'bgmi'
        (handle && handle.includes(sfx)) ||   // e.g. 'paramx-hack' includes 'param-x'
        (handle && sfx.includes(handle)) ||   // reverse: role includes the handle
        (code   && code === sfx)         ||   // exact code match e.g. 'bgmi' === 'bgmi'
        (code   && sfx.includes(code) && code.length > 2) || // role contains code (avoid 1-2 char false matches)
        (title  && title.includes(sfx) && sfx.length > 2);   // title match guard against short sfx
...
const EventManager: React.FC<EventManagerProps> = ({ adminProfile, isSuper }) => {
  ...
  const [uploading, setUploading] = useState<Record<string, number | null>>({}); // key -> progress %
  const fileInputRefs = useRef<Record<string, HTMLInputElement | null>>({});
  // Guard: ensures fetchOverrides runs exactly once — prevents re-fetches from wiping local edits
  const overridesLoaded = useRef(false);

  // Load allowed events and write assignedEventSlugs to admin doc for backend enforcement
  useEffect(() => {
    const keys = resolveEventKeys(adminProfile, isSuper);

    let filtered: Competition[];
    if (isSuper) {
      // Superadmin sees ALL competitions — no slug/id filtering
    ...
    // Write assigned keys to admin doc so Firestore backend rule can enforce per-event access
  ...
  // Load Firestore overrides for all events — runs ONCE only after events are first set.
  // Using a ref guard prevents re-fetches from overwriting the user's in-progress local edits.
  useEffect(() => {
    ...
    const fetchOverrides = async () => {
      ...
      await Promise.all(events.map(async (ev) => {
        ...
        } catch (e) { /* no Firestore override yet — will use static data */ }
      }));
      setOverrides(loaded);
  ...
  const handleSave = async (ev: Competition) => {
    ...
    // Frontend guard: verify the slug is in the admin's allowed list
    // allowedSlugs is empty for superadmins (means unrestricted)
    ...
    setSaving(key);
    try {
      // Build the save payload — strip undefined values because Firestore rejects them.
      // ev.slug and ev.handle may be undefined for competitions that only have an id.
      ...
      // Remove any other undefined values that may have crept in from overrides
      const savedData = Object.fromEntries(
        Object.entries(rawData).filter(([, v]) => v !== undefined)
      );

      await setDoc(doc(db, 'events_content', key), savedData, { merge: true });
      // Sync local overrides state with saved data so edits persist without re-fetch
  ...
  const setCoordinator = (key: string, idx: number, field: 'name' | 'phone', value: string) => {
    const coords = JSON.parse(JSON.stringify(getField(key, 'coordinators', [{ name: '', phone: '' }])));
